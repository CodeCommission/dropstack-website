...
export default class Docs extends React.Component {
  static async getInitialProps (req, res) {
    const fs = require('fs')
    const docPage = req.params.param1 || 'getting-started';
    const markdownFilePath = `docs/${docPage}.md`
    const source = fs.existsSync(markdownFilePath) ? fs.readFileSync(markdownFilePath).toString() : ''
  ...
  render() {
    return (
      <div>
        <Helmet>
          <title>DROPSTACK.RUN › Documentation</title>
        </Helmet>
        <SecondaryNav>
          <NavItems>
